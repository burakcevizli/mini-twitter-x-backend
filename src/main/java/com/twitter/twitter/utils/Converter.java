...
public class Converter {
    public static UserResponse userResponseConverter(User user){
        return new UserResponse(user.getFirstName(),user.getLastName(), user.getEmail(), user.getPassword(),
                user.getPhone(), user.getUserName(), user.getBirthday() , user.getRegisterDate() , user.getAddress(),
    ...
    public static TweetResponse tweetResponseConverter(Tweet tweet){
        return new TweetResponse(tweet.getText(),tweet.getLikes(),tweet.getRetweet(),tweet.getCommentsTotal(),
                tweet.getTweetDate(),userTweetResponseConverter(userResponseConverter(tweet.getUser())));
    }
